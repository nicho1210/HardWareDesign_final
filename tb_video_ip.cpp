...
int main()
{
    ...
    // x = 640  -> third column
    // y = 360  -> third row
    // region index = 10 (0-based), region number = 11
    const int hot_x = 640;
    const int hot_y = 360;
    const int num_pixels = FRAME_WIDTH * FRAME_HEIGHT;
    const unsigned expected_region_bit = (1u << 10);   // region 11
    ...
    video_gray_live_frame(in_stream, out_stream, &motion_info_out, num_pixels);

    errors += drain_and_check_frame(out_stream, -1, -1, 255, 0);

    unsigned count_f1 = (unsigned)(motion_info_out & 0xFFFF);
    unsigned mask_f1  = (unsigned)((motion_info_out >> 16) & 0xFFFF);
    ...
    // -----------------------------
    // Frame 2: one bright sampled block in region 11
    // -----------------------------
    ...
    unsigned mask_f2  = (unsigned)((motion_info_out >> 16) & 0xFFFF);
    ...
    if (mask_f2 != expected_region_bit) {
        std::cout << "Frame 2 region_mask mismatch: got=0x"
                  << std::hex << mask_f2
                  << " expect=0x" << expected_region_bit
                  << std::dec << std::endl;
        ++errors;
    } else {
        std::cout << "Frame 2 region_mask = 0x"
                  << std::hex << mask_f2 << std::dec
                  << " (region 11)" << std::endl;
    }
