...
int main(void)
{
    ...
    Status = system_init(&VtcInst, &ResetGpioInst, &RxDebugGpioInst, &TxDebugGpioInst);
    if (Status != XST_SUCCESS) {
    ...
    xil_printf("[DBG ] rx_lock = %lu, tx_lock = %lu\r\n",
    ...
    while (1) {
        ...
        xil_printf("[HB  ] app alive=%lu, rx_lock=%lu, tx_lock=%lu\r\n",
        ...
        if ((prev_rx_lock == 0) && (rx_lock == 1) && (tx_lock == 0)) {
            ...
            xil_printf("[REC ] after VTC restart + reset pulse #%lu: rx_lock=%lu, tx_lock=%lu\r\n",
                       (unsigned long)reset_pulse_count,
                       (unsigned long)rx_lock,
                       (unsigned long)tx_lock);
        }
